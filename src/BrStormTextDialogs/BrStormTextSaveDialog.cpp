/**
 * @brief BrStormTextLexerSaveDialog.cpp: A file of the StormText project.
...
#include "BrStormTextSaveDialog.hpp"
...
BrStormTextSaveDialog::BrStormTextSaveDialog(QString fileName, QString currentLexer, BrStormTextTab *tab, QWidget *parent)
    : QFileDialog(parent)
{
    m_tab = tab;
...
void BrStormTextSaveDialog::saveFile()
{
    ...
    if (f.open(QFile::WriteOnly | QFile::Text))
    {
        ...
        m_success = true;
        m_text = selectedFiles().at(0);

        m_tab->setTabToolTip(m_tab->currentIndex(), fi.absoluteFilePath());
        m_tab->setTabText(m_tab->currentIndex(), fi.fileName());
        m_tab->currentTab()->setModified(false);

        QFileIconProvider fip;
        QIcon icns = fip.icon(fi);
        m_tab->setTabIcon(m_tab->currentIndex(), icns);
    }
...
bool BrStormTextSaveDialog::isSuccessfulSaving()
{
    return m_success;
}

QString BrStormTextSaveDialog::currentText()
{
    return m_text;
}
